...
import copy
import re
from consts import ENDPOINT_DELIMITER, COUNTER_DELIMITER
from models import TmpGraph
from monitor_api import Endpoint, EndpointCounter
...
def generate_graph_urls(graph, start, end):
    ...
    if not counters:
        return []
    endpoint_list = graph.hosts.split(ENDPOINT_DELIMITER) or []
    ...
    counters = []
    for c in graph.counters.split(COUNTER_DELIMITER):
        if c.find("metric=") == -1:
            counters.append(c)
        else:
            metric = ""
...
def _generate_graph_urls(graph, counters, endpoint_list, start, end):
    ...
    if graph.graph_type == 'h':
        for c in counters:
            ...
            new_g.counters = c
            if end:
                new_g.src = '''/monitor/chart/h?id=%s&start=%s&end=%s''' % (tmp_graph_id, start or (0-graph.timespan), end)
            else:
                new_g.src = '''/monitor/chart/h?id=%s&start=%s''' % (tmp_graph_id, start or (0-graph.timespan))
            if graph.method == 'SUM':
                new_g.src += "&sum=on"
            else:
        ...
        for e in endpoint_list:
            ...
            new_g.hosts = e
            if end:
                new_g.src = '''/monitor/chart/k?id=%s&start=%s&end=%s''' % (tmp_graph_id, start or (0-graph.timespan), end)
            else:
                new_g.src = '''/monitor/chart/k?id=%s&start=%s''' % (tmp_graph_id, start or (0-graph.timespan))
            if graph.method == 'SUM':
                new_g.src += "&sum=on"
            else:
                new_g.src += "&cf=%s" % graph.method
        ...
        new_g = copy.deepcopy(graph)
        if end:
            new_g.src = '''/monitor/chart/a?id=%s&start=%s&end=%s''' % (tmp_graph_id, start or (0-graph.timespan), end)
        else:
            new_g.src = '''/monitor/chart/a?id=%s&start=%s''' % (tmp_graph_id, start or (0-graph.timespan))
        if graph.method == 'SUM':
            new_g.src += "&sum=on"
        else:
            new_g.src += "&cf=%s" % graph.method
...
def create_tmp_graph(endpoints, counters):
    ...
    id_ = TmpGraph.objects.create(endpoints=ENDPOINT_DELIMITER.join(endpoints), counters=COUNTER_DELIMITER.join(counters),
