...
from opsplatform.api import logger
...
class MyRunner(MyInventory):
    ...
    def run(self, module_name='shell', module_args='', timeout=10, forks=10, pattern='*',
            become=False, become_method='sudo -s', become_user='root', become_pass='', transport='paramiko'):
        """
            ...
            for host, info in contacted.items():
                if info.get('invocation').get('module_name') in ['raw', 'shell', 'command', 'script']:
                    if info.get('rc') == 0:
                        ...
                        result['failed'][host] = info.get('stdout') if info.get('stdout') else '' + \
                                                                                               info.get('stderr') if info.get('stderr') else ''
                else:
                    if info.get('failed'):
                        result['failed'][host] = info.get('msg')
                    else:
...
class MyTask(MyRunner):
    ...
    def gen_sudo_script(role_list, sudo_list):
        ...
        sudo_j2 = get_template('perm/role_sudo.j2')
